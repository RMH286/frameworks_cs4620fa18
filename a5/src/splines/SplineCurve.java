...
public abstract class SplineCurve {
  ...
  private void setBeziers() {
    ...
    if (this.isClosed()) {
      this.bezierCurves.add(this.toBezier(this.controlPoints.get(this.controlPoints.size()-3),
                                          this.controlPoints.get(this.controlPoints.size()-2),
                                          this.controlPoints.get(this.controlPoints.size()-1),
                                          this.controlPoints.get(0),
                                          this.epsilon));
      this.bezierCurves.add(this.toBezier(this.controlPoints.get(this.controlPoints.size()-2),
                                          this.controlPoints.get(this.controlPoints.size()-1),
                                          this.controlPoints.get(0),
                                          this.controlPoints.get(1),
                                          this.epsilon));
      this.bezierCurves.add(this.toBezier(this.controlPoints.get(this.controlPoints.size()-1),
                                          this.controlPoints.get(0),
                                          this.controlPoints.get(1),
                                          this.controlPoints.get(2),
                                          this.epsilon));
    }
  }
	  ...
	  if(crossSection.isClosed()) {
  	  for(int i = 0; i < crossSection.getPoints().size(); i++) {
  			for(float j = 0; j < 2*Math.PI; j+=sliceTolerance) {
  				...
  				Vector3 normal = Matrix4.createRotationZ(j).mulPos(new Vector3(0,n.x,n.y)).normalize();
